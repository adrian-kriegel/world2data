...
Usage:
    uv run world2data-demo                                   # default video
    uv run world2data-demo --video data/inputs/video.mp4     # custom video
    uv run world2data-demo --usdview                         # open usdview after
    uv run world2data-demo --review                          # open review UI after
"""
import os
import sys
import json
...
# Resolve project root: src/world2data/pipeline/ -> four levels up
...
# =========================================================================
DEFAULT_VIDEO = str(_PROJECT_ROOT / "data" / "inputs" / "video_2026-02-08_09-36-42.mp4")
OUTPUT_DIR = str(_PROJECT_ROOT / "data" / "outputs" / "demo_output")
...
def run_demo(video_path: str, output_dir: str, use_multimodel: bool = True,
    ...
    from world2data.pipeline.controller import World2DataPipeline
    ...
    if pipeline.yolo_detections:
        try:
            from world2data.pipeline.scene_fusion import VideoAnnotator
...
def _launch_review_ui(review_json_path):
    ...
    try:
        subprocess.Popen([
            sys.executable, "-m", "world2data.pipeline.human_review_ui",
            "--json", review_json_path,
        ])
    except Exception as e:
        print(f"WARNING: Could not launch review UI: {e}")
        print(f"  Run manually: uv run python -m world2data.pipeline.human_review_ui --json {review_json_path}")
